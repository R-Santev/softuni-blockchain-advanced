    ...
    function checkOnERC1155Received(
        ...
        if (to.code.length > 0) {
            ...
            returns (bytes4 response) {
                ...
                // Security: Handle both empty and non-empty revert reasons
                if (reason.length == 0) {
                    // non-IERC1155Receiver implementer
                    revert IERC1155Errors.ERC1155InvalidReceiver(to);
                } else {
                    // Gas Optimization: Use assembly for efficient revert with reason
...
contract ERC1155 is
    ...
    IERC1155Errors
{
    // Gas Optimization: Use Arrays library for efficient array operations
    using Arrays for uint256[];
    using Arrays for address[];

    // Gas Optimization: Use nested mappings for efficient storage layout
    mapping(uint256 id => mapping(address account => uint256))
        private _balances;

    // Gas Optimization: Use mapping for O(1) operator approval checks
    ...
    function balanceOfBatch(
        ...
        // Gas Optimization: Pre-allocate array size to avoid resizing
        uint256[] memory batchBalances = new uint256[](accounts.length);

        // Gas Optimization: Use library functions for safe array access
    ...
    function _update(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            if (from != address(0)) {
                ...
                // Gas Optimization: Use unchecked for arithmetic operations where overflow is impossible
                unchecked {
            ...
            // Gas Optimization: Direct storage write for to address
            if (to != address(0)) {
                _balances[id][to] += value;
            }
        }

        // Gas Optimization: Emit appropriate event based on array length
        if (ids.length == 1) {
    ...
    function _updateWithAcceptanceCheck(
        ...
        // Security: Follow CEI pattern - update state before external call
    ...
    function _safeTransferFrom(
        ...
        // Gas Optimization: Use helper function to create singleton arrays
    ...
    function _setApprovalForAll(
        ...
        // Security: Validate operator address
    ...
    function _asSingletonArrays(
        ...
        // Gas Optimization: Use assembly for efficient array creation
        assembly ("memory-safe") {
